...
function Two(props) {
    ...
    const handle_post = async (e) => {
        ...
        axios.post(path + `/room/${id_coded}/submit/4`, room)
            .then(res => {
    ...
    return (
        <div>
            {sendingForm ? <>
                ...
                <div><div className='room_type_header' ><strong>Двоповерхове ліжко (1 поверх)</strong></div>
                    <div className="main_div" key={"main_form_div"}>
                        <form className="main_form" >
                            <div className='furniture_list' key="furniture_div">
                                ...
                                <input placeholder="Введіть ім'я та прізвище" className="user_input" type="text" key={"inp_user" + 1} onChange={(e) => handleChangeUser(1, e.target.value)} value={room.names[1]} />
                                <label className="text_header" key={"user_"}>
                                    <div>Дата поселення</div>
                                </label>
                                <input placeholder="ДД/ММ/PP" className="user_input" type="date" key={"inp_user" + 1} onChange={(e) => handleChangeStartDate(1, e.target.value)} value={room.start_dates[1]} />
                                <br></br>
